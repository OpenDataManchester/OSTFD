...
|itemIdentifier|`mandatory`|UUID|The unique identifier of the items that this component is made of. There must be an equivalent record in the `Base Materials`, `Materials`, `Components`, `Textile and Fashion Products`, `Multipacks` OR `Load` data.|
|measureIdentifier|`mandatory`|ID|The measure type of this item. The entry here should be drawn from the [meausures](../controlled-lists/measures.md).|
|value|`mandatory`|Decimal|The value of this measurement. |
